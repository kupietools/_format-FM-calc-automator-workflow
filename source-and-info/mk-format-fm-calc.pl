...
sub writeresult {
    ...
    # went wrong instead, and exit -- rather than shipping output that can't be trusted.
    #
    # Exit 0, not a nonzero code: confirmed via the real `automator` CLI (not just direct perl
    # invocation) that Automator's "Run Shell Script" action treats a nonzero exit as the ACTION
    # ITSELF failing and discards the script's entire stdout -- the user would see a generic, empty
    # "The action 'Run Shell Script' encountered an error" with none of this diagnostic text, which
    # defeats the entire point of writing it. Exit 0 with this message as the only stdout content is
    # confirmed (same tool) to pass through correctly as the Service's replacement text.
